          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                ...
                href={link.href}
                className={cn(
                  "px-4 py-2 rounded-xl text-sm font-semibold font-body transition-all duration-200",
                  isActive(link.href)
                    ? "text-violet-deep bg-lavender-100"
                    : "text-slate-600 hover:text-violet-deep hover:bg-lavender-100"
                )}
              >
                {link.label}
              </Link>
            ...
            <div className="flex flex-col gap-1">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={cn(
                    "px-4 py-3 rounded-xl text-sm font-semibold font-body text-left transition-colors",
                    isActive(link.href)
                      ? "text-violet-deep bg-lavender-100"
                      : "text-slate-700 hover:bg-lavender-100"
                  )}
                >
                  {link.label}
                </Link>
